...
internal static class GridLayerRenderer
{
    ...
    /// <summary>
    /// Rounds <c><paramref name="worldSpan"/> / <paramref name="targetLineCount"/></c> up to the
    /// nearest 1, 2 or 5 multiple of a power of ten. This gives "nice" grid intervals that stay
    /// stable (no drift) while panning and change in discrete steps while zooming.
    /// </summary>
    /// <param name="worldSpan">The visible world span (world units) for the wider viewport dimension.</param>
    /// <param name="targetLineCount">Desired number of grid lines; actual count will vary.</param>
    internal static double CalcNiceStep(double worldSpan, int targetLineCount)
    {
