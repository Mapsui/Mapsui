...
namespace Mapsui.Fetcher
{
    public class TileFetcher : INotifyPropertyChanged
    {
        ...
        private readonly int _maxRequests = int.MaxValue;
        ...
        private void FetchMissingTiles(List<TileInfo> missingTiles, CancellationToken globalCancellationToken)
        {
            ...
            {
                if (!_tilesInProgress.Add(info.Index)) continue;
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private List<TileInfo> GetTilesNotInCache(List<TileInfo> tilesNeeded)
        {
        ...
        public long NumberOfTimesLoopStarted;
        ...
        public void ViewChanged(BoundingBox newExtent, double newResolution)
        {
            ...
            if (_fetchLoopTask == null || _fetchLoopTask.IsCompleted || _fetchLoopCancellationTokenSource.IsCancellationRequested)
            {
                ...
                NumberOfTimesLoopStarted++;
                Debug.WriteLine($"New loop {NumberOfTimesLoopStarted}");

                _fetchLoopTask = Task.Run(() => TileFetchLoop(fetchLoopCancellationToken, _fetchLoopCancellationTokenSource), fetchLoopCancellationToken);
            }
        }

        public long IterationsInLoop;

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        void TileFetchLoop(CancellationToken globalCancellationToken, CancellationTokenSource globalCancellationTokenSource)
        {
            ...
            while (!globalCancellationToken.IsCancellationRequested)
            {
                ...
                if (_viewportChanged)
                {
                    ...
                    // assign needed tiles to missing tiles and work with the missing tiles.
                    // This set will become smaller and smaller over every iteration as long
                    // as the viewport does not change  This has a big impact on performance.
                ...
                IterationsInLoop++;
                Debug.WriteLine($"New loop {IterationsInLoop}");
            }
        }
    }
}
