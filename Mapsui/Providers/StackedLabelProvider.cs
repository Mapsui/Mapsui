    ...
    public class StackedLabelProvider : IProvider
    {
            ...
            IEnumerable<IFeature> features, Pen line, Brush fill)
        {
            ...
            foreach (var cluster in clusters)
            {
                ...
                var orderedFeatures = cluster.Features.OrderBy(f => f.Geometry.GetBoundingBox().Centroid.Y);
            ...
            double resolution)
        {
            ...
            foreach (var feature in features.OrderBy(f => f.Geometry.GetBoundingBox().Centroid.Y))
            {
                ...
                var found = false;
                foreach (var cluster in clusters)
                    if (cluster.Box.Grow(minDistance).Contains(feature.Geometry.GetBoundingBox().Centroid))
                    {
