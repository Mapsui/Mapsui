...
REM nuget push .\Artifacts\Mapsui.Uno.WinUI.%VERSION%.nupkg -source nuget.org || exit /B 1
nuget push .\Artifacts\Mapsui.WinUI.%VERSION%.nupkg -source nuget.org || exit /B 1
REM git commit -m %VERSION% -a || exit /B 1
REM git tag %VERSION% || exit /B 1
REM git push origin %VERSION% || exit /B 1
REM git push || exit /B 1
