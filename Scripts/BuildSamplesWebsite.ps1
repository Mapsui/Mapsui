...
# change base url
(Get-Content -Path "release/wwwroot/index.html") -replace '<base href="/" \/>', '<base href="/v4/samples/" />' | Set-Content -Path "release/wwwroot/index.html"
...
New-Item -Path "website/v4/samples" -ItemType Directory -Force
# copy it to website/samples
Copy-Item -Path "release/wwwroot/*" -Destination "website/v4/samples" -Recurse -Force
