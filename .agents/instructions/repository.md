# Mapsui repository instructions

These instructions guide any coding agent working in this repository.
...
- Build and test the affected projects according to the validation policy in `AGENTS.md`; do not use an unscoped solution build as the default validation command.
- Full-checklist validation includes the supported Android projects and therefore requires the Android SDK and .NET Android workloads.
- Check formatting with the same `dotnet format ... --verify-no-changes` commands used by CI.
...
## Contributions agents should optimize for
...
The experimental renderer (`Mapsui.Experimental.Rendering.Skia`) wraps RichTextKit (RTK) inside `SkiaTextLayoutHelper`. RTK remains a required dependency — it provides UAX#14 line breaking, BiDi, and font fallback. When measuring line height, always use `font.Spacing` (ascent + descent + leading) rather than tight glyph bounds. See `.agents/instructions/richtextkit.instructions.md` for full RTK guidance, or load the `.agents/skills/richtextkit/SKILL.md` skill for on-demand reference.

## After making changes — checklist

Before making code changes, use the validation assessment in `AGENTS.md`. Load the checklist skill (`.agents/skills/checklist/SKILL.md`) only for partial or full checklist validation.

## Available skills

| Skill | When to load |
|---|---|
| `.agents/skills/checklist/SKILL.md` | When `AGENTS.md` selects partial or full checklist validation |
| `.agents/skills/create-pull-request/SKILL.md` | When opening a pull request — covers title prefix conventions, PR type label selection, and the correct tool to use |
| `.agents/skills/regression-tests/SKILL.md` | When working with rendering regression tests: running, updating references, diagnosing failures, or adding experimental-renderer samples |
| `.agents/skills/richtextkit/SKILL.md` | When implementing text rendering features involving word wrap, BiDi/RTL, emoji, font fallback, or multi-style text blocks |
...
- Ensure CI passes; run the builds and tests selected by the validation policy before pushing.
