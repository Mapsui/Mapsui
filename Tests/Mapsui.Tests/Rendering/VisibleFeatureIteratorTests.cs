...
namespace Mapsui.Tests.Rendering
{
    ...
    {
        [Test]
        public void TestIfStyleInStyleCollectionApplies()
        {
            // Arrange
            var viewport = new Viewport { Width = 100, Height = 100, Resolution = 1 };
            var vectorStyle = new VectorStyle();
            using var memoryLayer = new MemoryLayer { Style = new ThemeStyle(f => new StyleCollection { Styles = { vectorStyle } }) };
            ...
            // Assert
            Assert.IsTrue(result[feature].Contains(vectorStyle));
        }
