    ...
    public class BitmapAtlasSample : ISample
    {
        ...
        public static List<IFeature> CreateFeatures()
        {
            ...
            return new List<IFeature>
            {
                ...
                {
                    Styles = new[] {new ImageStyle {BitmapId = atlas} }
                },
                new PointFeature(new MPoint(20, 280))
                {
                    Styles = new[] {new ImageStyle {BitmapId = BitmapRegistry.Instance.Register(spriteAmusementPark15)} }
                },
                new PointFeature(new MPoint(60, 280))
                {
                    Styles = new[] {new ImageStyle { BitmapId = BitmapRegistry.Instance.Register(spriteClothingStore15)} }
                },
                new PointFeature(new MPoint(100, 280))
                {
                    Styles = new[] {new ImageStyle { BitmapId = BitmapRegistry.Instance.Register(spriteDentist15)} }
                },
                new PointFeature(new MPoint(180, 300))
                {
                    Styles = new[] {new ImageStyle { BitmapId = BitmapRegistry.Instance.Register(spritePedestrianPolygon)} }
                },
                new PointFeature(new MPoint(380, 280))
                {
                    Styles = new[] {new ImageStyle { BitmapId = svgTigerBitmapId, SymbolScale = 0.1} }
                }
            };
        }
    }
