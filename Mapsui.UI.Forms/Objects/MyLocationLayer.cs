    ...
    public void UpdateMyLocation(Position newLocation, bool animated = false)
    {
        if (!MyLocation.Equals(newLocation))
        {
            ...
            if (animated)
            {
                ...
                if (_mapView.Map.Navigator.State.ToExtent() is not null)
                {
                    var fetchInfo = new FetchInfo(_mapView.Map.Navigator.State.ToSection(), _mapView.Map?.CRS, ChangeType.Discrete);
